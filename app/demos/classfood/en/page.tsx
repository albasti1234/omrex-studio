...
function Hero({ t, lang }: { t: typeof en; lang: string }) {
    return (
...
function MenuHub({ t, lang }: { t: typeof en; lang: string }) {
...
function MenuCard({ item, t, lang, onAdd }: { item: MenuItem; t: typeof en; lang: string; onAdd: () => void }) {
...
function ItemDetailsSheet({ item, t, lang, onClose, onAdd }: { item: MenuItem; t: typeof en; lang: string; onClose: () => void; onAdd: () => void }) {
    return (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center" onClick={onClose}>
            ...
            >
                ...
                <div className="flex gap-2 mb-6">
                    {item.spicy && <span className="bg-red-100 text-red-600 px-3 py-1 rounded-full text-sm">ðŸŒ¶ï¸ Spicy</span>}
                    {item.veg && <span className="bg-green-100 text-green-600 px-3 py-1 rounded-full text-sm">ðŸ¥¬ Vegetarian</span>}
                    {item.sweet && <span className="bg-pink-100 text-pink-600 px-3 py-1 rounded-full text-sm">ðŸ° Sweet</span>}
                </div>
...
function FloatingCart({ t, lang }: { t: typeof en; lang: string }) {
    const cart = useCart();
...
function CheckoutSheet({ t, lang, onClose }: { t: typeof en; lang: string; onClose: () => void }) {
    const cart = useCart();
    ...
    const sendWhatsApp = () => {
        ...
        cart.items.forEach(item => {
            message += `â€¢ ${item.nameEn} x${item.quantity} - ${(item.price * item.quantity).toFixed(1)} AED%0A`;
        });
                        ...
                        <div className="bg-gray-50 rounded-xl p-4 text-left mb-6">
                            {cart.items.map(item => (
                                <div key={item.id} className="flex justify-between py-1">
                                    ...
                                    <span>{(item.price * item.quantity).toFixed(1)} AED</span>
                                </div>
                            ))}
...
function Experience({ t }: { t: typeof en }) {
    return (
...
function Reviews({ t }: { t: typeof en }) {
    return (
...
function Location({ t }: { t: typeof en }) {
    return (
...
function Footer({ t, lang }: { t: typeof en; lang: string }) {
    return (
